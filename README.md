...
```text
.
├── Formula_detection.ipynb    # メインのJupyter Notebook
├── SHIKIdata.yaml             # データセット設定ファイル（自動生成）
├── yolo_synthetic_data/       # 生成された合成データセット
│   ├── images/                # 学習・検証用画像
│   └── labels/                # YOLOフォーマットのアノテーション
├── dataset/                   # 学習用に分割されたデータセット
│   ├── train/                 # 訓練用 (80%)
│   └── val/                   # 検証用 (20%)
├── runs/                      # YOLOv8の学習ログ・結果（自動生成）
│   └── detect/
│       └── math_structure_model/
│           └── weights/
│               └── best.pt    # 学習済みモデル（最高精度）
├── shiki_models/              # 保存済みモデル管理用フォルダ
│   └── shiki_structure_yolo_v8.pt
└── README.md
